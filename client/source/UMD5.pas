...
function MD5(const s:TStream):string;overload;

  // вычисление хеш-суммы для строки
function MD5String(const S: string): TMD5Digest;

// вычисление хеш-суммы для файла
function MD5File(const aFileName: string;aBeforeCopyName:string = ''): TMD5Digest;
...
// сравнение двух файлов по MD5
function MD5FileCompare(const aFileName1,aFileName2:string):boolean;
...
function MD5(const s:TSTream):string;overload;
begin
    result:= MD5DigestToStr(MD5Stream(s));
end;
function MD5String(const S: string): TMD5Digest;
begin
...
begin
    cRaise:=false;
    try
        cFile := TFileStream.Create(aFileName, fmOpenRead);
        try
        try
            result := MD5Stream(cFile);
        except
            cRaise:=true;
        end
        finally
    ...
    if (cRaise) and (aBeforeCopyName<>'')  then
    begin
        if CopyFile(PWideChar(aFileName),PWideChar(aBeforeCopyName),false) then
        begin
            cFile := TFileStream.Create(aBeforeCopyName, fmOpenRead);
            try
                result := MD5Stream(cFile);

            finally
                cFile.Free;
                DeleteFile(aBeforeCopyName);
            end;
...
function MD5FileCompare(const aFileName1,aFileName2:string):boolean;
const
			cFuncName = 'MD5FileCompare';
var
    cDigest1, cDigest2: TMD5Digest;
begin
    // сравнение двух файлов по MD5
    result:=false;
    if (not FileExists(aFileName1)) or (not FileExists(aFileName2)) then
        exit;

    try
    try
        cDigest1:=MD5File(aFileName1);
        cDigest2:=MD5File(aFileName2);

        result:=MD5DigestCompare(cDigest1,cDigest2);
